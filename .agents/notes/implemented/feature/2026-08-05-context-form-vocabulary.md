...
**`catalog`** — a catalog of items available this session, republished as it changes. `dsh-tool-skill` moves off the shared `plugin` kind to its own `skill-catalog` source carrying `entries` (the exact `name`/`description` pairs published), `routingRevision` for the model-selection guidance, and `update` on a replacement, which the body renders as a replacement notice. The body lists those entries instead of re-parsing the `<available_skills>` block out of the prose.
...
That move also relocates catalog **identity**: the republish digest covers the durable entries and `routingRevision` rather than the rendered text, so ordinary model-facing framing edits cannot decide whether a republish is needed, and the text-slicing that recovered entries from a logged message is gone. A model-selection rule that must reach resumed sessions increments the revision; historical messages without the field are revision zero and republish once. One case does not self-heal: if an unreadable old-format catalog is the only one and the current view has no skills, the plugin sees no published catalog and emits no tombstone, so the model keeps a stale catalog nothing replaces. The pre-release stance ("backends reject old on-disk formats") permits it; it is recorded here rather than left to the optimistic path.
