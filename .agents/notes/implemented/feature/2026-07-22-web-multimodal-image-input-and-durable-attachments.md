...
Compaction replays the selected conversation prefix, including image references, into the configured summarization route. A native visual route resolves those references through its adapter; an explicitly text-only route uses the installed automatic fallback or fails explicitly instead of silently dropping the visual context. The synthesized checkpoint remains text-only, and `compaction-basic` rejects image summary output with `UNSUPPORTED_CONTENT`.
...
- Adapter and compaction tests cover native Pi-AI image conversion, late attachment-service composition, text-only rejection without fallback, durable direct and nested fallback projection, preserved summary input, and explicit image-output rejection.
