...
- **Registration is composition-conditional, execution is route-gated.** The tool registers only under `ctx.inject(['attachments'], …)` — no store, no tool. At execution, before any I/O, the strict gate resolves the calling route (latest `request/header` config, falling back to agent options) through `ctx.llm.resolveImageInput` and accepts native image input or the [automatic durable fallback](2026-08-17-automatic-vision-fallback.md); unknown capability and text-only routes without a ready fallback refuse. A refusal is a plain `isError` result, so an unsupported route's durable history never acquires an image block and the session cannot brick its own route.
