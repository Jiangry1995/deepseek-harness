...
Every returned snapshot records its originating tab in the Service Worker. Document-bound actions resolve that retained `pageId` before consulting the current side-panel tab, so switching tabs between read and action cannot redirect an operation. `wait-page` accepts the latest `pageId` as its preferred target and accepts `tabId` only when no snapshot exists. A missing or conflicting snapshot binding is `BROWSER_PAGE_STALE`, not a fallback to another tab. The side-panel tab projection uses a monotonic generation so a slower lookup for an older activation cannot overwrite the latest title or focused-tab report.
...
Fill still uses native prototype setters plus `input`/`change` for input and textarea, and Chromium editing commands for contenteditable. After filling, the actor reads the actual control value and fails when it does not match. Password, OTP, payment, file, hidden, disabled, and read-only controls remain unreadable and unfillable.

The page-bridge protocol version is 5. Service Worker, content script, page script, and Web Client share that version. Result `kind` must match the request `kind`.
...
The model can compose generic browser tools against the current page without site-specific adapters. Wait, scroll, and bounded keys increase the automation surface, so result verification and stale-ref rejection become more important. Snapshot-to-tab routing is in-memory; a Service Worker restart makes a pageId-only wait stale and requires another read. Screenshots, coordinate clicks, debugger permission, and moving the Browser Provider into the Service Worker remain later deliveries. Protocol 5 is a breaking bump; source changes must regenerate `extension/*.js` and the aggregated Remote Client before reload.
