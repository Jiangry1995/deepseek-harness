...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  {
    key: 'browser',
    summary: 'Routes browser operations to the most recently healthy WebExtension provider.',
    description: 'Routes browser operations to the most recently healthy WebExtension provider. Provider registrations are leases: expiry, disconnect, service disposal, caller cancellation, and request timeout all settle every affected pending operation.',
    methods: [
      {
        signature: '@Remote(\'connect\') connect(rawClientId: string, visible: boolean): BrowserClientLease',
        description: 'Register or renew a WebExtension provider identity.',
        parameters: [{ name: 'rawClientId', description: 'untrusted candidate identity generated by the Web Client.' }, { name: 'visible', description: 'whether the provider\'s page is renderer-visible and can answer promptly.' }],
        returns: 'the validated identity and timing values the Client must honor.',
      },
      {
        signature: '@Remote(\'heartbeat\') heartbeat(clientId: BrowserClientId, visible: boolean): BrowserClientLease',
        description: 'Renew one registered provider lease.',
        parameters: [{ name: 'clientId', description: 'identity returned by {@link connect}.' }, { name: 'visible', description: 'whether the provider\'s page is renderer-visible and can answer promptly.' }],
        returns: 'the renewed lease timing values.',
        throws: ['BROWSER_CLIENT_NOT_CONNECTED when the lease is absent or expired.'],
      },
      {
        signature: '@Remote(\'disconnect\') disconnect(clientId: BrowserClientId): BrowserDisconnectReceipt',
        description: 'Remove one provider and reject its outstanding operations.',
        parameters: [{ name: 'clientId', description: 'identity returned by {@link connect}.' }],
        returns: 'whether a current provider lease was removed.',
      },
      {
        signature: '@Remote(\'complete\') complete(completion: BrowserCompletion): BrowserCompletionReceipt',
        description: 'Complete one Host request from its selected extension provider.',
        parameters: [{ name: 'completion', description: 'echoed request and provider identities plus success or failure.' }],
        returns: 'whether the completion matched and settled a pending request.',
      },
      {
        signature: 'resolveOpenTab(request: BrowserOpenTabRequest): BrowserOpenTabSpec',
        description: 'Resolve caller defaults and validate one open-tab request.',
        parameters: [{ name: 'request', description: 'URL and optional activation preference.' }],
        returns: 'a complete provider operation.',
      },
      {
        signature: 'async openTab(request: BrowserOpenTabRequest, signal: AbortSignal): Promise<BrowserTab>',
        description: 'Open one HTTP(S) tab through the selected extension provider.',
        parameters: [{ name: 'request', description: 'URL and optional activation preference.' }, { name: 'signal', description: 'caller cancellation.' }],
        returns: 'the created tab.',
      },
      {
        signature: 'async listTabs(signal: AbortSignal): Promise<BrowserTab[]>',
        description: 'List tabs in the extension\'s current browser window.',
        parameters: [{ name: 'signal', description: 'caller cancellation.' }],
        returns: 'tabs visible to the extension.',
      },
      {
        signature: 'async readPage(requestOrSignal: BrowserReadPageRequest | AbortSignal, signal?: AbortSignal): Promise<BrowserPage>',
        description: 'Read bounded visible text and non-secret form values from one browser page.',
        parameters: [{ name: 'requestOrSignal', description: 'optional tab identity, or the caller AbortSignal for the active tab.' }, { name: 'signal', description: 'caller cancellation when the first argument is a request record.' }],
      ...
      {
        signature: 'resolvePageId(rawPageId: string): BrowserPage[\'pageId\']',
        description: 'Validate and brand a snapshot identity returned by the latest page read.',
        parameters: [{ name: 'rawPageId', description: 'page UUID supplied at the model-tool boundary.' }],
        returns: 'the branded snapshot identity.',
      },
      {
        signature: 'async clickPage(target: BrowserPageTarget, signal: AbortSignal): Promise<BrowserPageActionReceipt>',
        description: 'Click one element referenced by the latest current-page snapshot.',
        parameters: [{ name: 'target', description: 'validated page and element identities.' }, { name: 'signal', description: 'caller cancellation.' }],
        returns: 'confirmation of the completed click.',
      },
      {
        signature: 'async fillPage(request: BrowserFillPageRequest, signal: AbortSignal): Promise<BrowserPageActionReceipt>',
        description: 'Fill one text field referenced by the latest current-page snapshot.',
        parameters: [{ name: 'request', description: 'validated target, replacement value, and submit preference.' }, { name: 'signal', description: 'caller cancellation.' }],
        returns: 'confirmation of the completed fill.',
      },
      {
        signature: 'async selectPage(request: BrowserSelectPageRequest, signal: AbortSignal): Promise<BrowserPageActionReceipt>',
        description: 'Select one native option referenced by the latest current-page snapshot.',
        parameters: [{ name: 'request', description: 'validated target and exact option value or visible text.' }, { name: 'signal', description: 'caller cancellation.' }],
        returns: 'confirmation including the resolved native option value.',
      },
      {
        signature: 'async scrollPage(request: BrowserScrollPageRequest, signal: AbortSignal): Promise<BrowserScrollReceipt>',
        description: 'Scroll the document viewport or one scroll target from the latest page read.',
        parameters: [{ name: 'request', description: 'validated page identity, optional scroll-target ref, and discrete movement.' }, { name: 'signal', description: 'caller cancellation.' }],
        returns: 'observed offsets after the scroll attempt, including an at-boundary result.',
      },
      {
        signature: 'async focusPage(target: BrowserPageTarget, signal: AbortSignal): Promise<BrowserPageActionReceipt>',
        description: 'Focus one field or focusable action from the latest page read.',
        parameters: [{ name: 'target', description: 'validated page and element identities.' }, { name: 'signal', description: 'caller cancellation.' }],
        returns: 'confirmation that document.activeElement is the referenced element.',
      },
      {
        signature: 'async pressPage(request: BrowserPressPageRequest, signal: AbortSignal): Promise<BrowserPageActionReceipt>',
        description: 'Press one allowed key against a referenced element from the latest page read.',
        parameters: [{ name: 'request', description: 'validated target, allowed key, optional modifiers, and repeat count.' }, { name: 'signal', description: 'caller cancellation.' }],
        returns: 'confirmation of the completed key effect.',
      },
      {
        signature: 'resolvePressPage(request: BrowserPressPageRequest): BrowserPressPageSpec',
        description: 'Validate and default one bounded keyboard request.',
        parameters: [{ name: 'request', description: 'target, key, optional modifiers, and optional repeat count.' }],
        returns: 'a complete provider operation.',
      },
      {
        signature: 'async waitPage(request: BrowserWaitPageRequest, signal: AbortSignal): Promise<BrowserPage>',
        description: 'Wait until a page condition holds, then return a fresh snapshot.',
        parameters: [{ name: 'request', description: 'tab identity, condition, and optional timeout bounds.' }, { name: 'signal', description: 'caller cancellation.' }],
        returns: 'a new page snapshot after the condition is observed.',
      },
      {
        signature: 'resolveWaitPage(request: BrowserWaitPageRequest): BrowserWaitPageSpec',
        description: 'Validate and default one wait-page request.',
        parameters: [{ name: 'request', description: 'tab identity, condition, and optional timeout bounds.' }],
        returns: 'a complete provider operation.',
      },
      {
        signature: 'async activateTab(tabId: number, signal: AbortSignal): Promise<BrowserTab>',
        description: 'Activate one tab in its browser window.',
        parameters: [{ name: 'tabId', description: 'browser-assigned tab identifier.' }, { name: 'signal', description: 'caller cancellation.' }],
        returns: 'the activated tab.',
      },
      {
        signature: 'async closeTab(tabId: number, signal: AbortSignal): Promise<{ tabId: number; closed: true }>',
  ...
  },
  {
    ...
    methods: [
      ...
      {
        signature: 'registerImageFallback(fallback: LlmImageFallback): () => void',
        description: 'Register the process\'s sole automatic image-to-text fallback. The registration is released with its Cordis fiber.',
        parameters: [{ name: 'fallback', description: 'provider that logs and projects image-bearing requests.' }],
        returns: 'disposer that removes this exact provider.',
      },
      {
        signature: 'async resolveImageInput( provider: string, model: string, signal?: AbortSignal, ): Promise<ImageInputResolution>',
        description: 'Resolve whether one exact route handles images natively, through the installed fallback, not at all, or with unknown capability.',
        parameters: [{ name: 'provider', description: 'registered provider route.' }, { name: 'model', description: 'exact model id on the route.' }, { name: 'signal', description: 'cancellation for model and fallback availability lookup.' }],
  ...
  },
  {
    ...
    methods: [
      ...
      {
        signature: 'async probeSearch( providerId: string, signal?: AbortSignal, ): Promise<{ providerId: string; sourceCount: number }>',
        description: 'Probe one registered search provider by id, bypassing configured selection. Runs a minimal search so the settings surface can verify credentials and endpoint reachability without changing which provider the seam selects.',
        parameters: [{ name: 'providerId', description: 'registry id (`tavily`, `firecrawl`, …).' }, { name: 'signal', description: 'optional cancellation signal forwarded to the provider.' }],
  ...
  {
    key: 'webNetworking',
    summary: 'Host service that keeps each agent\'s live tool mask in sync with the session log.',
    description: 'Host service that keeps each agent\'s live tool mask in sync with the session log. Restrictions are process-local and must be re-applied after resume; the log is the durable source of truth.',
    methods: [
      {
        signature: 'sync(agent: Agent): void',
        description: 'Align the live restrict mask with the folded session preference.',
        parameters: [{ name: 'agent', description: 'the agent whose inherited web tools to mask or restore.' }],
      },
    ],
  },
  {
...
export const EVENT_API: readonly EventApiEntry[] = [
  ...
  },
  {
    ...
    mode: 'emit',
    signature: '\'browser/command\'(command: BrowserCommand): void',
    summary: 'Deliver one browser command to the selected Web Client provider.',
    description: 'Deliver one browser command to the selected Web Client provider.',
    parameters: [{ name: 'command', description: 'request identity, provider identity, and validated operation.' }],
  },
  {
...
export const TYPE_API: readonly TypeApiEntry[] = [
  ...
  },
  {
    ...
    declaration: 'export type BrowserClientId = Branded<\'BrowserClientId\'>;',
  },
  {
    name: 'BrowserClientLease',
    declaration: 'export interface BrowserClientLease {\n    clientId: BrowserClientId;\n    leaseMs: number;\n    requestTimeoutMs: number;\n}',
  },
  {
    name: 'BrowserCommand',
    declaration: 'export interface BrowserCommand {\n    requestId: BrowserRequestId;\n    clientId: BrowserClientId;\n    operation: BrowserOperation;\n}',
  },
  {
    name: 'BrowserCompletion',
    declaration: 'export interface BrowserCompletion {\n    requestId: BrowserRequestId;\n    clientId: BrowserClientId;\n    response: {\n        ok: true;\n        value: BrowserOperationResult;\n    } | {\n        ok: false;\n        error: {\n            code: BrowserExtensionErrorCode;\n            message: string;\n        };\n    };\n}',
  },
  {
    name: 'BrowserCompletionReceipt',
  ...
  {
    name: 'BrowserDocumentId',
    declaration: 'export type BrowserDocumentId = Branded<\'BrowserDocumentId\'>;',
  },
  {
    name: 'BrowserExtensionErrorCode',
    declaration: 'export type BrowserExtensionErrorCode = \'BROWSER_INVALID_REQUEST\' | \'BROWSER_TAB_NOT_FOUND\' | \'BROWSER_PAGE_ACCESS_DENIED\' | \'BROWSER_PAGE_STALE\' | \'BROWSER_ELEMENT_NOT_FOUND\' | \'BROWSER_ELEMENT_DISABLED\' | \'BROWSER_ELEMENT_NOT_EDITABLE\' | \'BROWSER_OPTION_NOT_FOUND\' | \'BROWSER_SCROLL_TARGET_INVALID\' | \'BROWSER_KEY_UNSUPPORTED\' | \'BROWSER_WAIT_TIMEOUT\' | \'BROWSER_CAPABILITY_UNAVAILABLE\' | \'BROWSER_API_FAILED\';',
  },
  {
    name: 'BrowserFillPageRequest',
    declaration: 'export interface BrowserFillPageRequest extends BrowserPageTarget {\n    value: string;\n    submit?: boolean;\n}',
  },
  {
    name: 'BrowserFillPageSpec',
  ...
  {
    name: 'BrowserOpenTabSpec',
    declaration: 'export interface BrowserOpenTabSpec {\n    kind: \'open-tab\';\n    url: string;\n    active: boolean;\n}',
  },
  {
  ...
  },
  {
    ...
    declaration: 'export interface BrowserPageActionReceipt extends BrowserPageTarget {\n    action: \'clicked\' | \'filled\' | \'selected\' | \'focused\' | \'pressed\';\n    value?: string;\n    key?: BrowserPressKey;\n}',
  },
  {
    name: 'BrowserPageElementRef',
    declaration: 'export type BrowserPageElementRef = Branded<\'BrowserPageElementRef\'>;',
  },
  {
    name: 'BrowserPageField',
    declaration: 'export interface BrowserPageField {\n    ref: BrowserPageElementRef;\n    label: string;\n    type: string;\n    value: string;\n    checked?: boolean;\n    disabled: boolean;\n    readOnly: boolean;\n    required: boolean;\n    inViewport: boolean;\n    focused: boolean;\n    context?: string;\n    options?: BrowserPageOption[];\n}',
  },
  {
    name: 'BrowserPageId',
  ...
  {
    name: 'BrowserPageScrollTarget',
    declaration: 'export interface BrowserPageScrollTarget {\n    ref: BrowserPageElementRef;\n    label: string;\n    axis: \'vertical\' | \'horizontal\' | \'both\';\n    top: number;\n    left: number;\n    maxTop: number;\n    maxLeft: number;\n}',
  },
  {
    name: 'BrowserPageTarget',
    declaration: 'export interface BrowserPageTarget {\n    pageId: BrowserPageId;\n    ref: BrowserPageElementRef;\n}',
  },
  {
    name: 'BrowserPageViewport',
    declaration: 'export interface BrowserPageViewport {\n    width: number;\n    height: number;\n    scrollX: number;\n    scrollY: number;\n    documentWidth: number;\n    documentHeight: number;\n}',
  },
  {
  ...
  },
  {
    ...
    declaration: 'export interface BrowserPressModifiers {\n    ctrl?: boolean;\n    alt?: boolean;\n    shift?: boolean;\n    meta?: boolean;\n}',
  },
  {
    name: 'BrowserPressPageRequest',
    declaration: 'export interface BrowserPressPageRequest extends BrowserPageTarget {\n    key: BrowserPressKey;\n    modifiers?: BrowserPressModifiers;\n    repeat?: number;\n}',
  },
  {
    name: 'BrowserPressPageSpec',
    declaration: 'export interface BrowserPressPageSpec extends BrowserPageTarget {\n    kind: \'press-page-key\';\n    key: BrowserPressKey;\n    modifiers: BrowserPressModifiers;\n    repeat: number;\n}',
  },
  {
    name: 'BrowserReadPageRequest',
    declaration: 'export interface BrowserReadPageRequest {\n    tabId?: number;\n}',
  },
  {
    name: 'BrowserRequestId',
    declaration: 'export type BrowserRequestId = Branded<\'BrowserRequestId\'>;',
  },
  {
    name: 'BrowserScrollMovement',
    declaration: 'export type BrowserScrollMovement = \'line-up\' | \'line-down\' | \'line-left\' | \'line-right\' | \'page-up\' | \'page-down\' | \'page-left\' | \'page-right\' | \'top\' | \'bottom\' | \'left-edge\' | \'right-edge\';',
  },
  {
    name: 'BrowserScrollPageRequest',
    declaration: 'export interface BrowserScrollPageRequest {\n    pageId: BrowserPageId;\n    ref?: BrowserPageElementRef;\n    movement: BrowserScrollMovement;\n}',
  },
  {
    name: 'BrowserScrollPageSpec',
    declaration: 'export interface BrowserScrollPageSpec extends BrowserScrollPageRequest {\n    kind: \'scroll-page\';\n}',
  },
  {
    name: 'BrowserScrollReceipt',
    declaration: 'export interface BrowserScrollReceipt {\n    pageId: BrowserPageId;\n    ref?: BrowserPageElementRef;\n    movement: BrowserScrollMovement;\n    top: number;\n    left: number;\n    maxTop: number;\n    maxLeft: number;\n    moved: boolean;\n    atBoundary: boolean;\n}',
  },
  {
    name: 'BrowserSelectPageRequest',
    declaration: 'export interface BrowserSelectPageRequest extends BrowserPageTarget {\n    value: string;\n}',
  },
  {
    name: 'BrowserTab',
  ...
  {
    name: 'BrowserWaitChangeCondition',
    declaration: 'export interface BrowserWaitChangeCondition {\n    kind: \'change\';\n    documentId: BrowserDocumentId;\n    afterRevision: number;\n}',
  },
  {
    name: 'BrowserWaitCondition',
    declaration: 'export type BrowserWaitCondition = BrowserWaitChangeCondition | BrowserWaitTextCondition | BrowserWaitUrlCondition | BrowserWaitReadyCondition;',
  },
  {
    name: 'BrowserWaitPageRequest',
    declaration: 'export interface BrowserWaitPageRequest {\n    kind: \'wait-page\';\n    pageId?: BrowserPageId;\n    tabId?: number;\n    condition: BrowserWaitCondition;\n    timeoutMs?: number;\n    stableMs?: number;\n}',
  },
  {
    name: 'BrowserWaitPageSpec',
    declaration: 'export interface BrowserWaitPageSpec {\n    kind: \'wait-page\';\n    pageId?: BrowserPageId;\n    tabId?: number;\n    condition: BrowserWaitCondition;\n    timeoutMs: number;\n    stableMs: number;\n}',
  },
  {
    name: 'BrowserWaitReadyCondition',
    declaration: 'export interface BrowserWaitReadyCondition {\n    kind: \'ready\';\n}',
  },
  {
    name: 'BrowserWaitTextCondition',
    declaration: 'export interface BrowserWaitTextCondition {\n    kind: \'text\';\n    text: string;\n    state: \'present\' | \'absent\';\n}',
  },
  {
    name: 'BrowserWaitUrlCondition',
    declaration: 'export interface BrowserWaitUrlCondition {\n    kind: \'url\';\n    value: string;\n    match: \'exact\' | \'prefix\' | \'contains\';\n}',
  },
  {
    name: 'CancelOptions',
  ...
  {
    name: 'ImageInputResolution',
    declaration: 'export type ImageInputResolution = {\n    kind: \'native\';\n} | {\n    kind: \'fallback\';\n} | {\n    kind: \'unsupported\';\n} | {\n    kind: \'unknown\';\n};',
  },
  {
    name: 'ImageMediaType',
