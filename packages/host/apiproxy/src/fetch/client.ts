...
import { webProbeSearchValueSchema } from '../api/web.schema.ts'
import {
...
export interface IApiClient {
  ...
  web: {
    probeSearch(payload: RequestPayload<'web.probeSearch'>, signal?: AbortSignal): Promise<RpcResponse<ResponseValue<'web.probeSearch'>>>
  }
...
const UNARY_VALUE_SCHEMAS: { [K in keyof RpcMethodMap]: z.ZodType<Wire<ResponseValue<K>>> } = {
  ...
  'web.probeSearch': webProbeSearchValueSchema,
}
...
export abstract class AbstractApiClient implements IApiClient {
  ...
  readonly web: IApiClient['web'] = {
    probeSearch: (payload, signal) => this.callUnary('web.probeSearch', payload, signal),
  }
