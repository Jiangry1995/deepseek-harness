...
本包还通过 `ToolDetails` 填充 `conversation.details.tool`。行 renderer 与详情 renderer 共用同一组面向 `terminal`、`read`、`diff`、`search` 和 `web` render intent 的纯 card model。未知的 intent 标签和格式错误的 wire card 数据都会回退为压平的工具结果文本。generic 路径上的图像块通过附件插件的 `conversation.message.images` 和 `conversation.details.images` slot 渲染；行缩略图位于折叠区之外，因此收起时只隐藏元数据信封，而压平后的 Output 会省略附件 JSON。

通用行把已知工具名称归类为 search、read、shell、write、edit、code 或 generic 变体；`read_image` 属于 read 家族，标题为 `Read image`。运行中、成功、失败和中断状态只来自冻结的 call/result slice。只有用户调用 Host 打开文件回调时，文件路径才相对会话 `cwd` 解析；展示代码不读取会话服务。
...
owner 载荷为 `ToolCallOwnerProps`：`callId`、`toolName`、冻结的 `block`、可选 `cwd` 与 `home`、普通的 `openFile`／`inspect` 回调，以及可选的附件 slot `renderImages` 回调。路径摘要先相对会话 cwd 缩短，再把剩余的 POSIX 宿主家目录写成 `~`；`filePath` 与 Host 打开仍使用作者给出的文件系统路径。注册项会收到常规的会话 slot 运行时共享数据，但不会收到运行时服务或 root/subcall 知识。
