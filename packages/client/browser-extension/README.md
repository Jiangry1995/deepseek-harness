...
The Client plugin registers only after that readiness response, renews the returned lease at half its duration, listens for addressed `browser/command` events, and completes each request through the Typert Remote browser namespace. Page-bridge requests use the Host timeout minus 1000ms of slack. `wait-page` uses `max(requestTimeoutMs, timeoutMs + 1500)` minus that slack so the in-page wait can finish first. Teardown stops heartbeats, rejects page-bridge work, waits for an in-flight registration, and disconnects any lease acquired during that race.

The extension supports `open-tab`, `list-tabs`, `read-page`, `inspect-page`, `click-page-element`, `fill-page-element`, `select-page-option`, `scroll-page`, `focus-page-element`, `press-page-key`, `wait-page`, `activate-tab`, and `close-tab`. Protocol version 7 is shared by the Service Worker, content script, page script, and Web Client. Ordinary HTTP(S) pages receive no static Side Assistant script. `read-page` and document-bound operations inject the isolated-world reader only into the selected tab, and each read observes only the first subsequent document mutation before disconnecting. `inspect-page` independently injects a dormant MAIN-world controller and requires `start`, `snapshot`, or `stop`; capture includes only later fetch/XHR and console calls, `snapshot` keeps it active, and `stop` releases owned wrappers and listeners. Console capture preserves strings and primitives but uses fixed tokens for objects instead of traversing them. Native DevTools and request or response bodies remain unavailable. Click, fill, select, scroll, focus, and press accept only coordinates from the latest snapshot and route through the tab retained for its `pageId`; a new read or navigation invalidates them. Fill verifies the actual control value after native setter or contenteditable editing events and can submit through the owning form, a nearby send control, or Enter. Select matches an exact native option value or visible text. Wait prefers a retained `pageId`, falls back after Service Worker state loss, and returns a fresh snapshot after document change, text, URL, or load stability. The bridge normalizes results to JSON, rejects malformed requests, bounds a complete page result to 96 KiB of serialized UTF-8, and maps page and Chromium failures to stable codes.

## Security constraints

The manifest grants `sidePanel` to open the assistant, `storage` to retain its selected origin, `nativeMessaging` for companion recovery, `tabs` for tab metadata and control, and `activeTab` plus `scripting` for on-demand page-script injection. Its HTTP(S) host permission declares read-and-operate access on ordinary sites without running code until a browser operation selects a tab. Extension-page `connect-src` and `frame-src` remain restricted to plain-HTTP `127.0.0.1` and `localhost`; the loopback content script uses `all_frames` so the embedded Web UI can register the provider. The Service Worker accepts Host operation messages only from this extension id and a loopback sender URL. Open-tab requests accept absolute credential-free HTTP(S) URLs; unsupported schemes and embedded credentials are rejected before `chrome.tabs.create`.
