...
扩展支持 `open-tab`、`list-tabs`、`read-page`、`inspect-page`、`click-page-element`、`fill-page-element`、`select-page-option`、`scroll-page`、`focus-page-element`、`press-page-key`、`wait-page`、`activate-tab` 和 `close-tab`。协议版本 6 由 Service Worker、content script、页面脚本和 Web Client 共用。`read-page` 可指定标签页或读取当前活动网页标签页，返回渲染文本、当前可见的非敏感表单值、焦点与 ARIA 状态、可点击元素、滚动目标、视口指标、快照 `pageId`、稳定 `documentId`、文档 `revision`，以及不透明引用。`inspect-page` 返回 MAIN 世界探针捕获的近期 fetch/XHR 请求和 console 消息；它不能打开原生 DevTools，也不返回请求或响应体。点击、填写、选择、滚动、聚焦和按键只接受这次快照中的坐标，并通过 `pageId` 保留的来源页签路由；再次读取或发生导航后，旧坐标立即失效。填写操作通过原生 input 或 textarea value setter 更新值并派发 `input` 与 `change`；contenteditable 编辑宿主则使用 Chromium 插入或删除编辑命令，以保留框架管理的子节点和原生输入事件。填写后会读取控件实际值，不一致则失败。只有 textbox ARIA role、但不具备浏览器可编辑性的元素不会作为字段公开。填写操作还可以提交所属表单、在没有表单时点击近邻的发送或提交控件、或派发 Enter，选择操作则按原生 option 的精确值或可见文本匹配。滚动在偏移未变化时返回已位于边界的结果。等待操作优先使用保留的 `pageId`。内存中绑定缺失时，与填写一样退回到焦点或当前活动页签；保留绑定与显式 `tabId` 冲突时仍视为过期。等待在文档变化、文本、URL 或加载稳定后返回新快照。桥接层会把结果规范化为 JSON、拒绝格式错误的请求、把完整页面结果限制在 96 KiB 序列化 UTF-8 以内，并把标签页不存在、引用过期、元素不可编辑、无效滚动目标、不支持的按键、等待超时、页面权限不可用和 Chromium API 失败映射为稳定错误代码。

## 安全约束

manifest（元数据清单）申请 `sidePanel` 以打开助手，申请 `storage` 以保存所选 origin，申请 `nativeMessaging` 以恢复伴随程序，申请 `tabs` 以读取标签元数据并控制标签页，并通过 `activeTab` 与 `scripting` 注入页面脚本。HTTP(S) host permission 和页面内容脚本构成扩展对普通站点明确声明的读取与操作权限。扩展页面的 `connect-src` 与 `frame-src` 仍只允许使用明文 HTTP 的 `127.0.0.1` 和 `localhost`；回环内容脚本启用 `all_frames`，让嵌入的 Web UI 能够注册提供方。Service Worker 只接受来自本扩展 id 且 sender URL 为回环地址的 Host 操作消息。打开标签页请求只接受不含凭据的绝对 HTTP(S) URL；不支持的协议和嵌入凭据会在调用 `chrome.tabs.create` 前被拒绝。
