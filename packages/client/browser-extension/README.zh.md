...
Client 插件仅在收到就绪响应后注册，在租约时长过半时续租，监听发给自身的 `browser/command` 事件，并通过 Typert Remote 浏览器命名空间完成每个请求。Teardown 会停止心跳、拒绝页面桥工作、等待正在进行的注册，并断开该竞态期间获得的任何租约。

扩展支持 `open-tab`、`list-tabs`、`read-page`、`click-page-element`、`fill-page-element`、`select-page-option`、`scroll-page`、`focus-page-element`、`press-page-key`、`wait-page`、`activate-tab` 和 `close-tab`。协议版本 5 由 Service Worker、content script、页面脚本和 Web Client 共用。`read-page` 可指定标签页或读取当前活动网页标签页，返回渲染文本、当前可见的非敏感表单值、焦点与 ARIA 状态、可点击元素、滚动目标、视口指标、快照 `pageId`、稳定 `documentId`、文档 `revision`，以及不透明引用。点击、填写、选择、滚动、聚焦和按键只接受这次快照中的坐标，并通过 `pageId` 保留的来源页签路由；再次读取或发生导航后，旧坐标立即失效。填写操作通过原生 input 或 textarea value setter 更新值并派发 `input` 与 `change`；contenteditable 编辑宿主则使用 Chromium 插入或删除编辑命令，以保留框架管理的子节点和原生输入事件。填写后会读取控件实际值，不一致则失败。只有 textbox ARIA role、但不具备浏览器可编辑性的元素不会作为字段公开。填写操作还可以提交所属表单或派发 Enter，选择操作则按原生 option 的精确值或可见文本匹配。滚动在偏移未变化时返回已位于边界的结果。等待操作优先使用保留的 `pageId`，尚无快照时也接受 `tabId`，并在文档变化、文本、URL 或加载稳定后返回新快照。桥接层会把结果规范化为 JSON、拒绝格式错误的请求、把完整页面结果限制在 96 KiB 序列化 UTF-8 以内，并把标签页不存在、引用过期、元素不可编辑、无效滚动目标、不支持的按键、等待超时、页面权限不可用和 Chromium API 失败映射为稳定错误代码。
